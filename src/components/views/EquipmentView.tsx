            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-2xs p-6 space-y-6 max-w-5xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-gray-100 pb-5">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full ${getOperationalStatusBadgeClass(selectedEquipment.status)}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-8 text-sm pt-2">
                ...
                <div>
                  ...
                  <span className="text-gray-900">{isBlankField(selectedEquipment.location) ? <NotSet /> : selectedEquipment.location}</span>
                </div>

                <div>
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">FOURNISSEUR</span>
                  <span className="text-gray-900">{isBlankField(selectedEquipment.supplier) ? <NotSet /> : selectedEquipment.supplier}</span>
                </div>

                <div>
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">FABRICANT</span>
                  <span className="text-gray-900 font-medium">{isBlankField(selectedEquipment.manufacturer) ? <NotSet /> : selectedEquipment.manufacturer}</span>
                </div>

                <div>
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">MODÈLE</span>
                  <span className="text-gray-900">{isBlankField(selectedEquipment.model) ? <NotSet /> : selectedEquipment.model}</span>
                </div>

                <div>
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">NUMÉRO DE SÉRIE</span>
                  <span className="text-gray-900">{isBlankField(selectedEquipment.serialNumber) ? <NotSet /> : selectedEquipment.serialNumber}</span>
                </div>
                ...
                <div className="md:col-span-3">
                  ...
                  <span className="text-gray-700 leading-relaxed">{isBlankField(selectedEquipment.description) ? <NotSet /> : selectedEquipment.description}</span>
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-gray-200">
                <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide">
                  Ordres de travail ({linkedWorkOrders.length})
                </h3>
                {linkedWorkOrders.length === 0 ? (
                  ...
                  <ul className="mt-3 max-h-80 overflow-y-auto divide-y divide-gray-100 border border-gray-200 rounded-lg">
                    {linkedWorkOrders.map(wo => (
                      <li key={wo.id} className="flex items-center justify-between gap-3 px-4 py-2.5 text-xs">
                        <div className="min-w-0">
                          <div className="font-semibold text-gray-900 truncate">{wo.title}</div>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          <span className="text-gray-500">{formatIsoDate(wo.dueDate)}</span>
                          <span className={`px-2 py-0.5 text-[10px] font-semibold border rounded-full ${getWorkOrderStatusBadgeClass(wo.status)}`}>
                            {wo.status}
                          </span>
                        </div>
                      </li>
